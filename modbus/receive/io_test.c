...
void parsing_rx_buff(void)
{
    ...
    if((UART_RX_BUFF[0] == 0xb8) && (UART_RX_BUFF[1] == 0xae))
    {
        if(UART_RX_BUFF[2] == 'O')
        {
            ...
            {
                command_data_no = UART_RX_BUFF[3];
                pwm_channel = UART_RX_BUFF[4];
            }
...
{
    IfxAsclin_Asc_read(&g_ascHandle,UART_RX_BUFF,&r_count, TIME_INFINITE);   /* Transmit data via TX */
    count++;
}
